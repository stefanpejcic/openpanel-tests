...
const DOMAIN = 'nextcloud.tests.openpanel.org';
...
test.describe('Nextcloud autoinstaller', () => {

  test('1. install app', async ({ page }) => {
    await page.goto('/nextcloud/install');
  ...
  test('3. verify app is responding', async ({ page }) => {
    ...
    const locator = page.getByText(/nextcloud/i);
    const timeout = 90000;
  ...
  test('5. remove app', async ({ page }) => {
    ...
    console.log('nextcloud uninstall is working');
  });
