#!/bin/bash
#
# Runs the OpenPanel (user panel) Playwright suite headlessly, on a single
...
LOG_FILE="$LOG_DIR/openpanel-${TIMESTAMP}.log"
JSON_FILE="$LOG_DIR/openpanel-latest.json"
...

{
  ...
  npx playwright test -c openpanel/playwright.config.ts --project=tests --workers=1 --reporter=list,html,json
  TEST_STATUS=$?
...
find "$LOG_DIR" -maxdepth 1 \( -name 'openpanel-*.log' -o -name 'html-report-openpanel-*' \) -mtime +30 -exec rm -rf {} +
